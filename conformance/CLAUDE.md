...
`builtin_gitlab_token.txt`, `builtin_jwt.txt` and `builtin_slack_token.txt`
(one pattern each), `builtins_together.txt` (all of them at once, and the values
two of them read differently), `custom_patterns.txt` (`MustRegexp`,
`NewPattern`, and no pattern at all), `overlap_and_attribution.txt` (how
overlapping values merge and which pattern the result is attributed to),
