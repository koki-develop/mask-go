...
//
// No key can be written inside another, which is the one place this pattern
// parts company with every built-in beside it. Each of those resumes a byte
// past a match because a value can begin inside the span of the one in front of
// it; here none can. A key begins only where no letter and no digit stands in
// front of it, and everything a span covers is one or the other except the
// underscores of the prefix — and none of the positions those underscores open
// opens a prefix of its own, since what stands at each of them is the rest of a
// mode or of the organization scope. The body cannot open one either: a prefix
// wants an underscore at its third character, and the third character of a body
// is a letter or a digit like the rest of it. So the spans of this pattern
// never overlap one another.
// Test_StripeAPIKey_noKeyBeginsInsideAnother drives every shape that would find
// that wrong.
//
